...
(function() {
  var BacklogDB, Bcrypt, BodyParser, CoffeeScript, Config, CookieParser, Crypto, Eco, Engine, Express, Fs, Gzippo, Http, Https, LocalStrategy, Log, MethodOverride, Morgan, Passport, PasswordHash, Path, ServeStatic, SessionStore, Url, User, Util, WebSocket, WorkingQueue, checkPassword, compact, count, del, ends, extend, flatten, last, merge, newPassword, starts, _, _ref;
  ...
  checkPassword = function(password, hash) {
    return PasswordHash.verify(password, hash) || Bcrypt.compareSync(password, hash);
  };
    ...
    Engine.prototype.startServer = function(port, callback) {
      ...
      this.app.post('/admin/users', restrict_admin, (function(_this) {
        return function(req, res) {
          ...
          return _this.db.insertUser(name, newPassword(password), isAdmin, function(row) {
            var user;
